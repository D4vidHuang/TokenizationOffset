...
# -*- coding: utf-8 -*-
"""
多语言 Rule-level Alignment Score 分析运行脚本
"""
...
def main():
    ...
    parser.add_argument('--code_dir', default='code_samples', help='代码样本目录')
    ...
    for language in languages_to_analyze:
        ...
        try:
            ...
            summary = analyzer.analyze_directory(str(lang_code_dir), str(lang_output_dir), language)
            
            if summary:
                all_results[language] = summary
                
                print(f"分析完成:")
                print(f"  文件数: {summary['analyzed_files']}")
                print(f"  平均 Rule-level Alignment Score: {summary['average_score']:.2f}%")
                print(f"  总规则数: {sum(r['total_rules'] for r in summary['file_results'].values())}")
                print(f"  总对齐规则数: {sum(r['aligned_rules'] for r in summary['file_results'].values())}")
                
                # 显示表现最好和最差的文件
                file_scores = [(f, r['rule_level_score']) for f, r in summary['file_results'].items()]
                if file_scores:
                    file_scores.sort(key=lambda x: x[1], reverse=True)
                    best_file, best_score = file_scores[0]
                    worst_file, worst_score = file_scores[-1]
                    
                    print(f"  最佳文件: {Path(best_file).name} ({best_score:.2f}%)")
                    print(f"  最差文件: {Path(worst_file).name} ({worst_score:.2f}%)")
                
                print(f"  详细报告已保存到: {lang_output_dir}")
            else:
                print(f"语言 '{language}' 分析失败")
...
def generate_cross_language_report(all_results, output_dir, model_name):
    ...
    for language, summary in all_results.items():
        total_files = summary['analyzed_files']
        avg_score = summary['average_score']
        total_rules = sum(r['total_rules'] for r in summary['file_results'].values())
        total_aligned = sum(r['aligned_rules'] for r in summary['file_results'].values())
